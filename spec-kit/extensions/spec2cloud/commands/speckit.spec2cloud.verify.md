---
description: Verify the implemented feature locally against real Azure resources by running `azd provision` first, then starting the components on the developer's machine wired to those resources.
handoffs:
  - label: Deploy to Azure    
    prompt: /speckit-spec2cloud-deploy
---
...
**Purpose.** Provision the feature's Azure dependencies via `azd provision`, then run the components locally pointed at those Azure resources so the user can exercise the feature end-to-end. A green run gates `/speckit-spec2cloud-deploy`; failure blocks it.
...
8. **Hand off.** Emit a clear **go / no-go** for `/speckit-spec2cloud-deploy` plus a short report:
   ...
   - Exact next step: `/speckit-spec2cloud-deploy` if all green, otherwise specific fixes.
